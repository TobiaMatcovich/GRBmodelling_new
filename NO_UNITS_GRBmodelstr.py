...
class GRBModel_topstruc:
    ...
    def __init__(self, eiso_zero, dens, tstart, tstop, redshift, pars, labels,
        ...
        # angles 
        self.theta_end_deg = theta_end.value   # truncation angle outside of which the energy is initially 0
        self.thetacore_deg = thetacore.value   # theta core of the jet in units of deg
        self.theta_obs_deg  = theta_obs.value  # viewing angle in units of grad

        self.theta_end_rad = theta_end.to("rad").value
        self.thetacore_rad = thetacore.to("rad").value
        self.theta_obs_rad  = theta_obs.to("rad").value
    ...
    def E_theta_gaussian(self,theta):
        ...
        thetacore_rad = self.thetacore_rad
        thetaw_rad = self.theta_end_rad
    ...
    def calc_photon_density(self, Lsy, sizereg,theta_inf,theta_sup):
        ...
        result=Lsy / (Omega_i * sizereg ** 2. * con.c )
        return result
    ...
    def _setup_shells(self):
        ...
        theta_min, theta_max = 0.0, self.theta_end_deg
        ...
        f_theta = lambda th_deg: doppler_factor(f_Gamma(th_deg),np.deg2rad(th_deg),self.theta_obs_rad,phi)
    ...
    def _SSCmodel_ind1fixed(self, pars, data):
        ...
        self.synch_comp_approx = np.zeros((n_shells, n_energy))
        self.ic_comp_approx = np.zeros((n_shells, n_energy))
        self.synch_comp = np.zeros((n_shells, n_energy))
        self.ic_comp = np.zeros((n_shells, n_energy)) 
        self.synch_compGG2 = np.zeros((n_shells, n_energy))
        self.ic_compGG2= np.zeros((n_shells, n_energy)) 

        model_wo_abs = np.zeros(n_energy) 
        ...
        size_reg = (self.sizer * u.cm).to(u.pc)               # size of the region as astropy quantity
        
        #--------------------------------------------------------------------------------------------------------------------------
        ampl = 1. / u.eV                              # temporary amplitude
        ECBPL = Models.ExponentialCutoffBrokenPowerLaw(ampl, 1e12 * u.eV, ebreak, alpha1, alpha2, e_cutoff)
        
        #---------------------------------------------- E min iterative process ----------------------------------------------------
        Emax = e_cutoff.value * 1e13 # maximum energy of the electron distribution, based on 10 * cut-off value in eV (1 order more then cutoff)
        Emin_0=1e9*u.eV
        Emin_0_exp=9
        energies = np.logspace(Emin_0_exp, np.log10(Emax), 100) * u.eV
        ...
        for i in range(n_shells):
          ...
          start=time.time()
          SYN = Radiative.Synchrotron(ECBPL, B=bfield, Eemin=emin, Eemax=Emax * u.eV, nEed=20)
          log.info(f"SYN class norm1: {time.time()-start}s")
          #----------------------------------------------------------------------------------------------------------------------------

          amplitude = ((eta_e * shock_energy * volume_element) / SYN.compute_Etot(Eemin=emin, Eemax=Emax * u.eV)) / u.eV

        
          ECBPL = Models.ExponentialCutoffBrokenPowerLaw(amplitude, 1e12 * u.eV, ebreak, alpha1, alpha2, e_cutoff)
          ...
          ph_density = self.calc_photon_density(Lsy, size_reg[i],theta_inf,theta_sup)   # number density of synchrotron photons (dn/dE) units of 1/eV/cm3
          log.info(f"ph density:{ph_density.unit}")
          
        #----------------------------------------------------------------------------------------------------------------------
          self.esycool = (synch_charene(bfield, ebreak))
          #self.synchedens = trapz_loglog(Esy * ph_density, Esy, axis=-1).to('erg / cm3')
              
          start=time.time()
          IC = Radiative.InverseCompton(ECBPL, seed_photon_fields=[['SSC', Esy, ph_density]], Eemin=emin, Eemax=Emax * u.eV, nEed=20)
          log.info(f"IC class: {time.time()-start}s")
          
          log.info("-------------------------------START : APPROX SPECTRUM ---------------------------------")
          start_synch = time.time()
          print((energy_grid *redf/ Gamma_i).unit)
          print(energy_grid.unit)
          synch_comp_approx=(Gamma_i** 2.) * SYN.sed(energy_grid.to(u.eV) *redf/ Gamma_i, distance=Dl)*redf
          #print("Synch approx max:",np.max(synch_comp_approx))
          ...
          start_ic = time.time()
          ic_comp_approx=(Gamma_i ** 2.) *  IC.sed(energy_grid.to(u.eV)* redf / Gamma_i, distance=Dl)*redf
          #print("IC approx max:",np.max(ic_comp_approx))
          ...
          D = doppler_factor(Gamma_i, np.deg2rad(theta_eff), self.theta_obs_rad, phi)  # array shape (n_phi,)
          ...
          #print("Synch max:",np.max(flux_syn_total))
          log.info(f"Tempo impiegato per integrare syn shell : {time.time() - start:.6f} s")
          
          start = time.time()
          flux_syn_total = trapz_numba(integrand_syn, phi) # shape (n_E,)
          #print("Synch max:",np.max(flux_syn_total))
          log.info(f"Tempo impiegato per integrare syn shell con trapz numba : {time.time() - start:.6f} s")

          start = time.time()
          flux_syn_total = simpson_uniform(integrand_syn, phi) # shape (n_E,)
          #print("Synch max:",np.max(flux_syn_total))
          log.info(f"Tempo impiegato per integrare syn shell con simpson : {time.time() - start:.6f} s")'''
          #------------------------------------------------
          start = time.time()
          flux_ic_total  = np.trapz(integrand_ic,  phi, axis=-1)
          #print("IC max:",np.max(flux_ic_total))
          log.info(f"Tempo impiegato per integrare ic shell : {time.time() - start:.6f} s")

          start = time.time()
          flux_ic_total  = trapz_numba(integrand_ic,  phi)
          #print("IC max:",np.max(flux_ic_total))
          log.info(f"Tempo impiegato per integrare ic shell con trapz_numba : {time.time() - start:.6f} s")
    ...
    def plot_jet_profile(self,bottom_order=3,Save=False,Name="Jet_profile",Path=None):
        
        theta = np.linspace(0, 30, 500)*u.deg
        plt.figure(figsize=(8,5))
        if self.energy_profile=='gaussian':
              E = self.E_theta_gaussian(theta)
              plt.plot(theta, E, label="Gaussiano troncato" )
        elif self.energy_profile == 'powerlaw':
              E = self.E_theta_powerlaw(theta,b=4.5)
    ...
    def print_GRB_status(self):
        ...
        line("Energy profile:",self.energy_profile)
    ...
    def plot_doppler_segmentation(self,phi_fixed,theta_max_deg, plot_Dphi=False,plot_Dmedio=False,Save=False,Path=None,Name=None):
        ...
        for i in range(len(theta_edges)-1):
          ...
          D_vals = doppler_factor(Gamma_array, theta_rad, np.deg2rad(self.theta_obs), np.deg2rad(phi_fixed))

          D_max = np.nanmax(D_vals)
          D_min = np.nanmin(D_vals)
