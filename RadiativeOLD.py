...
from Utils import trapz_loglog_nd
...
class InverseCompton(BaseElectron):
    ...
    """def _iso_ic_on_monochromatic(electron_energy, seed_energy, seed_edensity, gamma_energy):
        ...
        #gamma_energy = gamma_energy[:, None]   #1D
        #photE0 = photE0[:, None, None]         #1D
        #phn = phn[:, None, None]               #1D

        # Aggiungo dimensioni dummy per il broadcasting
        # electron_energy shape: (1, 1, n_e) se 1D
        electron_energy = np.atleast_2d(electron_energy)
        electron_energy = electron_energy[np.newaxis, np.newaxis, :]  # aggiunge due dimensioni iniziali

        # gamma_energy shape: (n_phi, n_gamma) -> aggiungo dimensione finale
        gamma_energy = np.atleast_2d(gamma_energy)
        gamma_energy = gamma_energy[..., np.newaxis]

        # seed photon shape: (n_ph,) -> (1, 1, n_ph)
        photE0 = photE0[np.newaxis, np.newaxis, :]
        phn = phn[np.newaxis, np.newaxis, :]
        ...
        if phn.size > 1:
            ...
            #gamint = trapz_loglog(gamint * phn / photE0, photE0, axis=0)  #1D
            gamint = trapz_loglog(gamint * phn / photE0, photE0, axis=-1) # 1/s
        else:
        ...
        #gamint *= (3.0 / 4.0) * sigt * c / electron_energy**2   #1D
        gamint *= (3.0 / 4.0) * sigt * c / electron_energy[..., 0]**2  # divisione sull’asse finale degli elettroni
    ...
    """def _iso_ic_on_monochromatic(electron_energy, seed_energy, seed_edensity, gamma_energy):
        ...
        IC cross-section for isotropic IC scattering on a monochromatic photon spectrum
        (Eq. 22 Aharonian & Atoyan 1981).
        Funziona con gamma_energy 1D, 2D o 3D.
        
        start = time.time()
        #print("electron_energy.shape:",electron_energy.shape)
        #print("seed_energy.shape:",seed_energy.shape)
        #print("seed_edensity.shape:",seed_edensity.shape)
        #print("gamma_energy.shape:",gamma_energy.shape)

        # Assicura array 1D
        electron_energy = np.atleast_1d(electron_energy)  # (Ne,)
        seed_energy     = np.atleast_1d(seed_energy)      # (Np,)
        seed_edensity   = np.atleast_1d(seed_edensity)    # (Np,)
        ...
        # ---- GAMMA ENERGY (può essere 1D,2D,3D) ----
        gamma_energy = np.asarray(gamma_energy)            # shape (..., Ng)
        
        # ---- Aggiungi assi per broadcasting ----
        # gamma: (..., 1, 1) → mantiene shape originale
        gamma_ = gamma_energy[..., None, None]            # (..., 1, 1)
        # elettroni: (1, Ne, 1) → broadcastabile su gamma e seed
        elec_  = electron_energy[None, :, None]          # (1, Ne, 1)
        # seed photons: (1, 1, Np)
        phot_  = photE0[None, None, :]                   # (1, 1, Np)
        phn_   = phn[None, None, :]     
        ...
        start5= time.time()
        b = 4 * phot_ * elec_  
        w = gamma_ / elec_  # gamma axis + electron axis
        q = w / (b * (1 - w))
        ...
        print(f"Tempo impiegato per preparazione variabili broadcasting : {time.time() - start5:.3f} s")

        sigt = 6.652458734983284e-25   # cm^2
        c = 29979245800.0  # cm/s
        elec2 = electron_energy[np.newaxis, :, np.newaxis]**2
        norm = (3.0/4.0) * sigt * c / elec2
        gamint = fic * norm
        norm = (3.0/4.0) * sigt * c / (elec_**2)
        
        gamint = fic * heaviside(1 - q) * heaviside(q - 1.0 / (4 * elec_**2))
        start3= time.time()
        gamint[np.isnan(gamint)] = 0.0
        print(f"Tempo impiegato per gamint[np.isnan(gamint)] = 0.0 : {time.time() - start3:.3f} s")
        gamint *= norm  

        # ---- Integrazione sul seed photon field ----
        if phn.size > 1:
            phn_ = phn_.to(1 / (mec2_unit * u.cm**3)).value
            #phn_ = phn.to(1 / (mec2_unit * u.cm**3)).value
            start1 = time.time()
            #gamint = trapz_loglog(gamint * phn_/ phot_, photE0, axis=-1)
            gamint = trapz_loglog_nd(gamint * phn_/ phot_, photE0)
            #gamint = trapz_loglog(gamint * phn_[np.newaxis, np.newaxis, :] / photE0[np.newaxis, np.newaxis, :], photE0, axis=-1)
